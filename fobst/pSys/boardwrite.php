...
<?php
	IF($_GET['idx']){
		$viewRow  = Result_Query("multiboard","pass,secret,title,contents","mysql_fetch_array","where idx='".$_GET['idx']."'");#게시판......
		IF(!$viewRow){ History_Back(-1,"No Data!"); }
		$SCheck   = NoticeChk($viewRow['secret']);		#비밀글체크
		$title=$viewRow['title'];
		$reContents="--------------------------------------------------------------------------- <br>제목: ".$viewRow['title']."<br>--------------------------------------------------------------------------- <br>내용: ".$viewRow['contents']."<br>---------------------------------------------------------------------------";
	}
...
?>
<form name="write_form" method="POST" enctype="multipart/form-data" action="./insertOk.php?idx=<?=$_GET['idx']?>&<?=$getView?>">
...
<table class="board_write">
  ...
  <tbody>
  	<!-- 2020-12-11 이태규 청년장기려 작성 시 년도 수정 -->
	<?if($bid=="fobst_const" || $bid=="lecture_lec" || $bid=="posit_photo3" || $bid=="posit_photo4" || $bid=="sec_photo4"){?> <!-- || $bid=="posit_photo5" -->
	<tr>
      <td class="tdl">년도</td>
      <td  class="tdr">
	  <select name="years">
	...
	<tr>
      ...
      <td  class="tdr">
			...
			<input type="checkbox" class="input_check fl mr5 mt2" name="notice" value="Y" /><span class="fl pl5 pt5 mr10">공지</span>
            <?}?>
	...
	<tr>
      ...
      <td  class="tdr"><input type="text" name="title" class="input_text fl"  style="width:550px;" value="<?//=$title;?>"/></td>
    </tr>
	<?if($bid!="BSC"){?>
	 <tr>
	...
	<?}?>
     <tr>
      ...
      <td  class="tdr"><input type="text" name="name" class="input_text fl mr5"  style="width:280px;" value="<?//=$_SESSION['sess_name']?>"/></td>
    </tr>
	<?if($bid=="BSC"){?>
     <tr>
      <td class="tdl">분류</td>
      <td  class="tdr"><input type="text" name="subject" class="input_text fl mr5"  style="width:280px;" value="<?//=$_SESSION['sess_name']?>"/></td>
    </tr>
     <tr>
      <td class="tdl">부서명</td>
      <td  class="tdr"><input type="text" name="depart" class="input_text fl mr5"  style="width:280px;" value="<?//=$_SESSION['sess_name']?>"/></td>
    </tr>
     <tr>
      <td class="tdl">동영상링크</td>
      <td  class="tdr"><input type="text" name="video" class="input_text fl mr5"  style="width:550px;" value="<?//=$_SESSION['sess_name']?>"/></td>
    </tr>
	<?}?>
     <tr>
      <td class="tdl"><?if($bid=="posit_photo3"  || $bid=="posit_photo5"){?>수상일<?}else{?>작성일<?}?></td>
      <td  class="tdr">
		<input type="text"   name="regdate"   style="width:280px;"  class="input_text fl mr5" id="datepicker" value="<?=date('Y-m-d');?>">
    ...
    <tr>
      <td class="tdl">내용</td>
      <td  class="tdr">
	  <textarea class="textarea_box" style="width:99%; height:200px;" name="contents" id="contents"><?=$reContents?></textarea>
		<script type="text/javascript">
			var oEditors = [];
	...
	<tr>
      <td class="tdl"><?if($bid=="lecture_lec"){?>강연제목<?}else{?>제목<?}?></td>
      <td  class="tdr"><input type="text" name="title" class="input_text fl"  style="width:550px;" value=""/></td>
    </tr>
     <tr>
      <td class="tdl">강연자</td>
      <td  class="tdr"><input type="text" name="name" class="input_text fl mr5"  style="width:280px;" value=""/></td>
    </tr>
     <tr>
      <td class="tdl">기관</td>
      <td  class="tdr"><input type="text" name="organ" class="input_text fl mr5"  style="width:280px;" value=""/></td>
    </tr>
     <tr>
      <td class="tdl">학과(소속)</td>
      <td  class="tdr"><input type="text" name="depart" class="input_text fl mr5"  style="width:280px;" value=""/></td>
    </tr>
     <tr>
      <td class="tdl">강연일</td>
      <td  class="tdr"><input type="text" name="lecDate" class="input_text fl mr5"  style="width:180px;" value=""/></td>
    </tr>
	<tr>
      <td class="tdl">강연지역</td>
      <td  class="tdr"><input type="text" name="region" class="input_text fl mr5"  style="width:180px;" value=""/></td>
    </tr>
	<tr>
      <td class="tdl">금과터강연영상보기</td>
      <td  class="tdr"><input type="text" name="siteUrl" class="input_text fl mr5"  style="width:550px;" value=""/></td>
    </tr>
	<?  }?>
  </tbody>
</table>
...
<div style="float:right">
	...
	<button class="btn_type01" style="width:70px;"  onclick="javascript:Write_Chk();return false;"><p><img src="../board/btn/save.png" /><strong>작성</strong></p></button>
</div>
</div>
</form>