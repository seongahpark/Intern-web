...
<?
	...
	//정규세션 3
	$getQuery = "SELECT idx FROM session WHERE del='N' AND kind='정규세션 3' ORDER BY idx LIMIT 1";
	...
	//정규세션 4
	$getQuery = "SELECT idx FROM session WHERE del='N' AND kind='정규세션 4' ORDER BY idx LIMIT 1";
	$res = mysql_query($getQuery, $conn);
	$res = mysql_result($res, 0);
	...
	//특별세션 3
	$getQuery = "SELECT idx FROM session WHERE del='N' AND kind='특별세션 3' ORDER BY idx LIMIT 1";
	$res = mysql_query($getQuery, $conn);
	$res = mysql_result($res, 0);
	...
	//특별세션 4
	$getQuery = "SELECT idx FROM session WHERE del='N' AND kind='특별세션 4' ORDER BY idx LIMIT 1";
	$res = mysql_query($getQuery, $conn);
	$res = mysql_result($res, 0);
	...
	//에필로그
	$getQuery = "SELECT idx FROM session WHERE del='N' AND kind='에필로그 세션' ORDER BY idx LIMIT 1";
	$res = mysql_query($getQuery, $conn);
	$res = mysql_result($res, 0);
	...
	$getQuery .= " WHERE idx='".$res."' LIMIT 1";
	$result = mysql_query($getQuery, $conn);
	$getKind_kor5 = mysql_result($result, 0, "kind");			//세선 종류
	$getTitle_kor5 = mysql_result($result, 0, "title");			//세션 명

	$getQuery = "SELECT * FROM ";
	$getQuery .= $tn2;
	$getQuery .= " WHERE idx='".$res."' LIMIT 1";
	$result = mysql_query($getQuery, $conn);
	$getKind_eng5 = mysql_result($result, 0, "kind");			//세선 종류
	$getTitle_eng5 = mysql_result($result, 0, "title");			//세션 명

	$getQuery = "SELECT * FROM ";
	$getQuery .= $tn3;
	$getQuery .= " WHERE idx='".$res."' LIMIT 1";
	$result = mysql_query($getQuery, $conn);
	$getKind_jpn5 = mysql_result($result, 0, "kind");			//세선 종류
	$getTitle_jpn5 = mysql_result($result, 0, "title");			//세션 명
?>
					...
					<table class="table_wrap">
						...
						<tbody>
							...
							<tr>
								<th>13:30 ~ 15:30</th>
								<td>
									<a href="live_forum03.php?lang=kor&kind=MS4">
										<p class="b_tit"><?=$getKind_kor2?></p>
										<p class="s_tit"><?=$getTitle_kor2?></p>
									</a>
								</td>
								<td>
									<a href="live_forum03.php?lang=eng&kind=MS4">
										<p class="b_tit"><?=$getKind_eng2?></p>
										<p class="s_tit"><?=$getTitle_eng2?></p>
									</a>
								</td>
								<td>
									<a href="live_forum03.php?lang=jpn&kind=MS4">
										<p class="b_tit"><?=$getKind_jpn2?></p>
										<p class="s_tit"><?=$getTitle_jpn2?></p>
									</a>
								</td>
								<th rowspan="2" class="m_clock">13:30 ~ 17:00</th>						
								<td rowspan="3" class="sp_td">
									<a href="live_forum03.php?lang=kor&kind=SS4">
										<p class="b_tit"><?=$getKind_kor4?></p>
										<p class="s_tit"><?=$getTitle_kor4?></p>
									</a>
								</td>
								<td  rowspan="3" class="sp_td">
									<a href="live_forum03.php?lang=eng&kind=SS4">
							...
							<tr>
								<th>15:30 ~ 16:00</th>
								<td class="n_hover break_col" colspan="3">Break</td>
							</tr>
							<!-- 3 -->
							<tr>
								<th>16:00 ~ 16:30</th>
								<td rowspan="2">
									<a href="live_forum03.php?lang=kor&kind=EP">
										<p class="b_tit"><?=$getKind_kor5?></p>
										<p class="s_tit"><?=$getTitle_kor5?></p>
									</a>
								</td>
								<td rowspan="2">
									<a href="live_forum03.php?lang=eng&kind=EP">
										<p class="b_tit"><?=$getKind_eng5?></p>
										<p class="s_tit"><?=$getTitle_eng5?></p>
									</a>
								</td>
								<td rowspan="2">
									<a href="live_forum03.php?lang=jpn&kind=EP">
										<p class="b_tit"><?=$getKind_jpn5?></p>
										<p class="s_tit"><?=$getTitle_jpn5?></p>
									</a>
								</td>
							</tr>
							<tr>
