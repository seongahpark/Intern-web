<? 
	...
	$session_date	= "2020-10-28";			  //세션 날짜 정보
	if($_GET) $lang	= $_GET['lang'];		  //세션 언어 정보
	else	  $lang = "kor";				  //default kor
	$tn				= "session_".$lang;		  //DB TABLE 이름

	if($_GET['kind'] == "MS1") $name = "정규세션 1"; //세션 종류 정보
	else if($_GET['kind'] == "MS2") $name = "정규세션 2"; //세션관리에서 구분할 수 있는 추가 인자가 딱히없어
	else if($_GET['kind'] == "SS1") $name = "특별세션 1"; //정확한 세션 구분을 위해서는 정확한 종류 입력이 필요
	else if($_GET['kind'] == "SS2") $name = "특별세션 2";			
...
function ajax_request(url) //이 함수는 Jquery를 사용하지 않은 AJAX
{
   ...
   xhr.open("GET", "log.php?date="+getDate+"&id="+id+"&session="+sessionEn+"&lang="+lang, true);
...
var timer = setInterval(function () { //인터벌 500인 타이머 생성후 코드 반복 실행
   ...
   if(didScroll){ //SCROLL 이벤트, 스크롤 사용시 하단으로 내려가는 것을 막기위함
	   	//console.log("scroll");
		didScroll = false;
   }else{
		$("#chatLog").scrollTop($("#chatLog")[0].scrollHeight);
   }
   }, 1000); //서버 부하 문제 고려로 1초로 수정
...
<?	if($_GET['kind'] == "MS1" || $_GET['kind'] == "MS2") { ?>
		<!--1-->
		<div class="sec04">
...
<?
	...
	if( $curr > '2020-10-28 09:30:00' && $curr < '2020-10-28 12:30:00'){ ?>
	...
	<?}else if($curr > '2020-10-28 13:00:00' && $curr < '2020-10-28 16:30:00'){ ?>
		<script>
