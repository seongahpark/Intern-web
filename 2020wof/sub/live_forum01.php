<? 
	...
	if($_GET['kind'] == "2020") $name = "2020 대한민국 해양인의 밤"; //세션 종류 정보
	else						$name = "개막식 & 기조세션"; 	
	...
	//echo $subdbDate;
	//echo "<script> console.log( $getKind  ); </script>";
	
	$selectSQL	= "SELECT * from statistics where ID = '".$member_ID."' AND session_type = '".$real_title."' AND language = '".$lang_convert."' AND stat_date = '".$subdbDate."' ";			//해당하는 ID의 회원의 수를 센다.
	...
	if(date_diff(new DateTime($curdate), $sessionDate)->format("%R%a") == 0){									// 세션 당일날 접속!
		if($count == 0){		//처음 들어온 사람일 시
			$insertSQL   = "insert into statistics (session_type , language, ID, belong, number, email, name, stat_date, time) ";
			$insertSQL  .= "values( '".$real_title."' , '".$lang_convert."' , '".$member_ID."' , '".$belong."' , '".$number."' , '".$email."' , '".$name."' , '".$curdate."' , '".$real_Time."' )";
			$resultConn  = mysql_query($insertSQL,$conn);
...
*/
	...
	// 2020 대한민국 해양인의 밤 가져오기
	$name			= "2020 대한민국 해양인의 밤";
...
function ajax_request(url) //이 함수는 Jquery를 사용하지 않은 AJAX
{
   ...
   xhr.open("GET", "log.php?date="+getDate+"&id="+id+"&session="+sessionEn+"&lang="+lang, true); //GET으로 시간, 세션 이름, 언어 정보 전송
...
<?
	...
	if( $curr > '2020-10-27 15:00:00' && $curr < '2020-10-27 17:30:00'){ ?>
	...
	<?}else if($curr >= '2020-10-27 17:30:00' && $curr < '2020-10-27 19:00:00'){ ?>
		<script>
